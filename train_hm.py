...
from model.swin_1_mediator import Pixel_Prediction, PatchMerging, Mediator #, Transformer
#from model.swin_2_mediator import Pixel_Prediction, PatchMerging, Mediator
...
class Train:
    def __init__(self, config):
        ...
        {'params': self.patchmerging1.parameters(), 'lr': self.opt.learning_rate, 'weight_decay': self.opt.weight_decay},
        #{'params': self.patchmerging2.parameters(), 'lr': self.opt.learning_rate,'weight_decay': self.opt.weight_decay},
    ...
    def create_model(self):
        ...
        self.patchmerging1 = PatchMerging(dim=256).cuda() # 3.28에 추가한 거 for merging
        self.mediator1 = Mediator(in_dim=512).cuda()
            ...
            if self.opt.load_epoch == -1:
                ...
                for file in os.listdir(models_dir):
                    ...
                    if file.split('.')[0].endswith('{}_best'.format(self.opt.d_name)):
                        ...
                        self.patchmerging1.load_state_dict(checkpoint['patchmerging1_state_dict'])
                        #self.patchmerging2.load_state_dict(checkpoint['patchmerging2_state_dict'])
    ...
    def train_epoch(self, epoch):
        ...
        self.patchmerging1.train()
        #self.patchmerging2.train()
        ...
        for data in tqdm(self.train_loader):
            ...
            "PatchMerging_1: (256,56,56) --> (512,28,28)"

            first_dis = self.patchmerging1(first_dis)
            first_ref = self.patchmerging1(first_ref)

            second_dis_ = self.mediator1(first_dis, second_ref) # 512, 28, 28 , 잔차
            second_ref_ = self.mediator1(first_ref, second_ref)


            pred = self.regressor(second_dis_, second_ref_, second_dis, second_ref)
        ...
        with torch.no_grad():
            ...
            for data in tqdm(self.val_loader):
                ...
                for i in range(self.opt.ensemble):
                    ...
                    pred += self.regressor(second_dis_, second_ref_, second_dis, second_ref) #딥에서 쉘로우로
                    #pred += self.regressor(second_dis, second_ref, second_dis_, second_ref_)
    ...
    def save_model(self, epoch, weights_file_name, loss, rho_s, rho_p):
        ...
        torch.save({
            ...
            'patchmerging1_state_dict': self.patchmerging1.state_dict(),
            #'patchmerging2_state_dict': self.patchmerging2.state_dict(),
